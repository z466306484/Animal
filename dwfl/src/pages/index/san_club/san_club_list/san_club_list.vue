      ...
      <div class="content">
        <div class="listbox">
          ...
          <div class="listbox_do">
            <div class="addbox">
              <el-button  size="small" :disabled="roleaddshow" type="primary" @click="addclub"><i class="el-icon-plus"></i>创建社群</el-button>
            </div>
            <div class="list_inputs">
              ...
              <!--按钮-->
              <el-button size="small " type="primary" @click="findclub">查找</el-button>
            </div>
          </div>
          <!--表格-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        //表格传递参数
        tableobj:{
          tharr:[{width:'5%',val:'id'},{width:'22%',val:'社群名称'},{width:'10%',val:'创建人'},{width:'10%',val:'社群人数'},{width:'17%',val:'动态数'},{width:'11%',val:'是否推荐'},{width:'5%',val:'状态'},{width:'20%',val:'操作'}],
          tabliearr:[],
          allarr:[],
          roleeditshow:true,
        },
    ...
    methods: {
      ...
      todetai(obj){
        if(obj.type==0){  //点击查看的方法
          this.$router.push({path:'/san_club_recover_show?type=1&&id='+obj.item.id})
        }
        if(obj.type==2){//点击编辑的方法
          ...
          this.$router.push({path:'/san_club_add?type=2&&id='+obj.item.id})
        }
      ...
      alterdetai(obj){
        ...
        if(obj.type==0){
          ...
          //  发请求
          this.axios.post(this.apis+'/san/community/del',{
            id:this.deleteid,
          }).then((res) => {
      ...
      findclub(){
        ...
        this.roleshow()
        this.axios.post(this.apis+'/san/community/search', {
      ...
      initdata(){
        let _this =  this
         this.axios.post(this.apis+"/san/community/indexshow")
        .then((res)=> {
           if(res.data.code==10000){
        ...
        this.axios.post(this.apis+'/san/Login/get_user').then((res) => {
          if(res.data.code==10000){
            ...
            for(var i=0;i<_this.rolebox.length;i++){
              if(this.rolebox[i].id==5){
                _this.roleaddshow=false
              }
              if(this.rolebox[i].id==6){
      ...
      addclub(){
        this.$router.push({path:'/san_club_add?type=1'})
      }
    },
    mounted(){
