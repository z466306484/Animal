<template>
  <div class="tablebox">
    <table class="table table-hover" style="margin-top: 20px;border: 1px solid #ddd;font-size: 12px;width: 100%">
      ...
      <tr v-for="item in tableobj.tabliearr" class="table_td">
        <td class="sortEdit" >
          <div class="imgdiv">
            <div>
              <img  style="width: 40px;height:40px;border-radius:20px;margin-top: 7px" :src="item.img">
            </div>
        ...
        <td style="display: flex;padding-top:20px;box-sizing: border-box;width: 200px;">
          <div style="margin-left: 5px" v-for="index in item.btnarr" class="bluebtn" @click="todetai(item,index.type)">{{index.val}}</div>
        </td>
      </tr>
      </tbody>
    </table>
