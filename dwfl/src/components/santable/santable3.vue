    ...
    <table class="table table-hover" style="margin-top: 20px;border: 1px solid #ddd;font-size: 12px;width: 100%">
      ...
      <tr v-for="(item,index) in tableobj.tabliearr" class="table_td">
        ...
        <td>
          <div class="imgdiv">
            <div class="showimg">
              <img style="width: 40px;height:40px;border-radius: 40px" :src="item.img">
            </div>
            <div class="person_p" v-for="index in item.answer">
              <div>
                <p >{{index.answer1}} <span style="color: #2e8ded;background-color: #eaf4fe">{{index.answer2}}</span></p>
                <p style="color:#999999">{{index.answer3}}</p>
              </div>
            </div>
          </div>
        </td>
        ...
        <td class="sortEdit" >{{item.time1}}</td>
        <td style="box-sizing: border-box;display:table-cell;vertical-align:middle;">
          <div style="margin-left: 5px;display:inline-block"  v-for="index in item.btnarr" v-bind:class="{ bluebtn: index.type=='0',blue1btn:index.type=='2',redbtn:index.type=='3'}" @click="todetai(item,index.type)">{{index.val}}</div>
        </td>
      </tr>
      </tbody>
    </table>
